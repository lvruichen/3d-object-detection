...
cv::RotatedRect LShapedFIT::FitBox(std::vector<cv::Point2f>* pointcloud_ptr)
{
    ...
    double dtheta =
        dtheta_deg_for_search_ * M_PI / 180;  // np.deg2rad(x) --> x * pi/180
    ...
    for (int k = 0; k < loop_number; ++k)
    {
        ...
        if (theta < (M_PI / 2.0 - dtheta))
        {
            ...
            {
                std::cout << "L-Shaped Algorithm Criterion Is Not Supported."
                          << std::endl;
                break;
            }
    ...
    if (minimal_cost > (-1.0) * std::numeric_limits<double>::max() &&
...
double LShapedFIT::calc_nearest_criterion(const cv::Mat& c1, const cv::Mat& c2)
{
    ...
    {
        double temp = std::min(sqrt(pow((c1_max - c1_deep[i]), 2)),
                               sqrt(pow((c1_deep[i] - c1_min), 2)));
        d1.push_back(temp);
    }

    for (int i = 0; i < n_c2; i++)
    {
        double temp = std::min(sqrt(pow((c2_max - c2_deep[i]), 2)),
                               sqrt(pow((c2_deep[i] - c2_min), 2)));
        d2.push_back(temp);
    }
...
double LShapedFIT::calc_variances_criterion(const cv::Mat& c1,
                                            const cv::Mat& c2)
{
    std::vector<double> c1_deep;
    std::vector<double> c2_deep;
    ...
    // D1 = [ min( [np.linalg.norm(c1_max - ic1), np.linalg.norm(ic1 - c1_min)]
    // ) for ic1 in c1 ]
    for (int i = 0; i < n_c1; i++)
    {
        double temp = std::min(sqrt(pow((c1_max - c1_deep[i]), 2)),
...
void LShapedFIT::calc_cross_point(const double a0, const double a1,
                                  ...
                                  const double c0, const double c1, double& x,
                                  double& y)
{
