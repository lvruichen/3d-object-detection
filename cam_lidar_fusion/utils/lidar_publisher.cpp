...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle nh;
    YAML::Node config = YAML::LoadFile("/home/eric/a_ros_ws/"
                                       "object_detection_ws/src/"
                                       "3d-object-detection/cam_lidar_fusion/"
                                       "config/image_lidar.yaml");
    cout << "lidar_path: " << config["lidar_path"].as<string>() << endl;
    string lidar_path = config["lidar_path"].as<string>();
    ros::Publisher lidar_pub =
        nh.advertise<sensor_msgs::PointCloud2>("/kitti/velo/pointcloud", 1);
