...
private:
    ...
    float color_distance;  // step length to color the lidar points according to
                           // plane distance(z)
    int frame_count = 0;

public:
    RsCamFusion(cv::Mat cam_intrinsic, cv::Mat lidar2cam_extrinsic,
                cv::Mat cam_distcoeff, cv::Size img_size, float color_dis,
                bool show_cloud, bool save)
    {
                  ...
                  const sensor_msgs::PointCloud2ConstPtr input_cloud_msg)
    {
        ...
        pcl::transformPointCloud(
            *input_cloud_ptr, *transformed_cloud,
            transform);  // lidar coordinate(forward x+, left y+, up z+)
                         // camera coordiante(right x+, down y+, forward z+)
                         // (3D-3D) using the extrinsic matrix between this two
                         // coordinate system
        ...
        for (int i = 0; i <= transformed_cloud->points.size(); i++)
        {
            ...
            {
                lidar_points.push_back(
                    cv::Point3d(transformed_cloud->points[i].x,
                                transformed_cloud->points[i].y,
                                transformed_cloud->points[i].z));
                int color_order =
                    int(transformed_cloud->points[i].z / color_distance);
                if (color_order > 20)
                {
                    color_order = 20;
                }
                dis_color.push_back(cv::Scalar(color[color_order][2],
                                               color[color_order][1],
                                               color[color_order][0]));
        ...
        // project lidar points from the camera coordinate to the image
        // coordinate(right x+, down y+)
        cv::projectPoints(lidar_points, rMat, tVec, intrinsic, distcoeff,
                          imagePoints);

        pcl::PointCloud<PointXYZRGBI>::Ptr colored_cloud(
            new pcl::PointCloud<PointXYZRGBI>);
        pcl::PointCloud<PointXYZRGBI>::Ptr colored_cloud_transback(
            new pcl::PointCloud<PointXYZRGBI>);
        cv::Mat image_to_show = input_image.clone();
        for (int i = 0; i < imagePoints.size(); i++)
        {
            if (imagePoints[i].x >= 0 && imagePoints[i].x < imageSize.width &&
                imagePoints[i].y >= 0 && imagePoints[i].y < imageSize.height)
            {
                cv::circle(image_to_show, imagePoints[i], 1, dis_color[i], 2, 8,
                           0);
                PointXYZRGBI point;  // reserve the lidar points in the range of
                                     // image
                point.x =
                    lidar_points[i].x;  // use 3D lidar points and RGB value of
                                        // the corresponding pixels
                point.y = lidar_points[i].y;  // to create colored point clouds
                point.z = lidar_points[i].z;
                point.r = input_image.at<cv::Vec3b>(imagePoints[i].y,
                                                    imagePoints[i].x)[2];
                point.g = input_image.at<cv::Vec3b>(imagePoints[i].y,
                                                    imagePoints[i].x)[1];
                point.b = input_image.at<cv::Vec3b>(imagePoints[i].y,
                                                    imagePoints[i].x)[0];
                point.i = intensity[i];
        ...
        pcl::transformPointCloud(*colored_cloud, *colored_cloud_transback,
                                 inv_transform);
        ...
        if (show_colored_cloud)
        {
            ...
            publishCloudtoShow(colored_cloud_showpub, cloud_header,
                               colored_cloud_toshow);
        }

        publishImage(fused_image_pub, image_header, image_to_show);
        if (save_data)
        {
            saveData(image_header, input_image, cloud_header, input_cloud_ptr,
                     colored_cloud_transback);
        }
    ...
    void saveData(const std_msgs::Header& image_header, const cv::Mat image,
                  const std_msgs::Header& cloud_header,
                  const pcl::PointCloud<pcl::PointXYZI>::ConstPtr& cloud,
                  pcl::PointCloud<PointXYZRGBI>::Ptr colored_cloud)
    {
        // timestamp: image_header.stamp.sec . image_header.stamp.nsec
        std::string img_name = std::to_string(frame_count) + "_" +
                               std::to_string(image_header.stamp.sec) + "_" +
                               std::to_string(image_header.stamp.nsec) + ".jpg";
        std::string cloud_name = std::to_string(frame_count) + "_" +
                                 std::to_string(cloud_header.stamp.sec) + "_" +
                                 std::to_string(cloud_header.stamp.nsec) +
                                 ".pcd";
        std::string colored_cloud_name =
            "c_" + std::to_string(frame_count) + "_" +
            std::to_string(cloud_header.stamp.sec) + "_" +
        ...
        pcl::io::savePCDFileASCII(
            colored_cloud_save_dir + "/" + colored_cloud_name, *colored_cloud);
    }

    void publishCloudtoShow(
        const ros::Publisher& cloudtoshow_pub, const std_msgs::Header& header,
...
void getExtrinsic(cv::Mat& lidar2cam_)
{
    ...
    try
    {
        tfListener.waitForTransform(camera_frame, lidar_frame, ros::Time::now(),
                                    ros::Duration(0.1));
        tfListener.lookupTransform(camera_frame, lidar_frame, ros::Time(0),
                                   lidar2camT);
    }
...
int main(int argc, char** argv)
{
    ...
    RsCamFusion fusion(cam_intrinsic, cam2lidar_extrinsic, cam_distcoeff,
                       img_size, color_dis, show_cloud, save_data);
    message_filters::Subscriber<sensor_msgs::Image> camera_sub(nh, camera_topic,
                                                               30);
    ...
    colored_cloud_showpub =
        nh.advertise<sensor_msgs::PointCloud2>("colored_cloud_toshow", 10);
    ros::spin();
    return 0;
}
